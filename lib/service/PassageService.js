...
module.exports = class {
  ...
  async findByCard(card, slaveOnly=true, limit = 5){
    const message = new Message();
    message.object = await this.history.findByCard(card, slaveOnly, limit);
    return message;
  }
  ...
  async accessMaster(cardId, lane){
    ...
    if(await this.history.add(cardId, lane)){
      message.object = await this.history.getLast(cardId);
      return message;
    }

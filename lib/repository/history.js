...
const basicSelector = 'select sequential_id, history.card_id, datetime(time, "+9 hours") as time , lane_id as ip , lane.name as lane_name, user.name as user_name '
const basicFrom = 'from history left outer join lane on history.lane_id=lane.ip left outer join user on history.card_id=user.card_id ' 
...
module.exports = class extends AbstractRepository {
  ...
  async getLast(cardId){
    const logs = await this.all(basicSelector + basicFrom + 'where history.card_id=? order by time desc, sequential_id desc limit 1', cardId)
    return logs[0]
  }
  async findByCard(cardId, slaveOnly = true, limit = 5, desc = true){
    let where = 'where history.card_id=?'
    const args = [cardId];
    if(slaveOnly){
      where += ' and kind=? '
      args.push(slave_lbl)

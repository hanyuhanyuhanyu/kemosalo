...
const basicSelector = 'select sequental_id, card_id, datetime(time, "+9 hours") as time , lane_id as ip , lane.name as lane_name '
...
module.exports = class extends AbstractRepository {
  ...
  async getLast(cardId){
    const logs = await this.all(basicSelector + basicFrom + 'where card_id=? order by time desc limit 1', cardId)
    return logs[0]
  }
  async findByCard(cardId, slaveOnly = true, limit = 5, desc = true){
    ...
    let sql = basicSelector + basicFrom + where + ' order by time' ;
    if(desc){
      sql += ' desc';
    }
    if(limit > 0){
